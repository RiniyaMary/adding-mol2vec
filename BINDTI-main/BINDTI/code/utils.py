...
# Add to utils.py (keep existing functions)
def graph_collate_func_with_smiles(x):
    """
    Collate function for Mol2Vec-enhanced dataloader
    """
    d, p, s, y = zip(*x)  # Unpack drug, protein, smiles, label
    d = dgl.batch(d)
    p = torch.tensor(np.array(p))
    s = torch.stack(s)  # Stack SMILES embeddings
    y = torch.tensor(y)
    return d, p, s, y
